...
public class StamOriginal : MonoBehaviour
{
    ...
    //-----------------------------------------------------------------
    // External forces (2 added for bounary cells)
    ...
    public void UpdateBounds( float[,] cur )
    {
        for( int i = 1; i <= c_Size; i++ )
        {
            cur[ 0, i ] = cur[ c_Size, i ];
            cur[ c_Size + 1, i ] = cur[ 1, i ];
            cur[ i, 0 ] = cur[ i, c_Size ];
            cur[ i, c_Size + 1 ] = cur[ i, 1 ];
        }
    }
    //-----------------------------------------------------------------
